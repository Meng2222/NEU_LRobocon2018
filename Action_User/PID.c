...
void PriorityControl(PID_Value *PID,Err *err,int targetn[])
{
	...
	PID->timeCnt ++ ;/*比赛计时*/
	if(err->flag == 1) return;/*避障时退出*/
	if(PID->timeCnt < 1500) return;/*15s内正常收球*/
	else if(PID->timeCnt < 18000)/*暂时设置为180s比赛时长*/
	{
		if(PID->dogHungry == 0 && PID->stop1 == 0)/*15s后有球去场地中央扫描射球*//*条件为未进入过无球状态*/
		{
				for(i = 0 ; i < 4 ; i ++ )/*更新走形优先级*/
				{
		...
		else/*15s后进入无球状态，先去扫边，再变轨扫内场，期间有球进入待发状态立即找最近的未投球桶投球，一条边上可投两个桶*/
		{
			PID->stop1 = 1;/*停止中场扫描模式，开始边收球边扫描投球*/
